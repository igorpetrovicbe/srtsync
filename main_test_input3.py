...
def get_files_in_folder(folder_path, suffix, input_suffix):
    sub_list = []
    input_sub_list = []
    vid_list = []
    txt_list = []
    ...
    for item in os.listdir(folder_path):
        ...
        if os.path.isdir(item_path):
            new_subs, new_vids, new_txts, new_input_subs = get_files_in_folder(item_path, suffix, input_suffix)
...
if __name__ == '__main__':
    ...
    for i in range(len(sub_path_list)):
        ...
        print(f"sub: {sub_path[-50:]} | vid: {vid_file_name} | {existance}")
    print(str(len(sub_path_list)) + ', ' + str(len(input_sub_path_list)))
    ...
    for i in range(start_from, len(sub_path_list)):
        ...
        if not skip_error_calc:
            ...
            first_line = 0
            for j in range(len(correct_sub)):
                if correct_sub[j].content == sub[0].content:
                    first_line = j
                    break

            for j in range(sample_size):
                sample_start_total += sub[j].start.total_seconds() - correct_sub[j+first_line].start.total_seconds()
